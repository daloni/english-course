...
- The `<link rel="canonical">` and the `og:url` end in a slash, the URL GitHub Pages serves
  without a redirect.
- `pnpm generate` runs `scripts/generate-sitemap.mjs` afterwards (`postgenerate`), which writes
  `sitemap.xml` and `robots.txt` (with its `Sitemap:` line) into `.output/public`.
...
indexing of the home. Optionally do the same in Bing Webmaster Tools. Search engines only read
`robots.txt` at the domain root, so the sitemap must be submitted by hand.
